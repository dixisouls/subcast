...
allowed-tools: Bash(python3 -m cli.spawn_agent:*), Agent
---
...
1. Run `python3 -m cli.spawn_agent context` from the project root. This
   prints the current permission context as JSON: `allowed_tools`,
...
2. Compare the task against each existing agent's `description` yourself,
   ...
   Optionally run `python3 -m cli.spawn_agent validate-verdict '<json>'`
   with your decision as `{"decision", "matched_agent_name", "confidence",
...
5. Before writing, run `python3 -m cli.spawn_agent requires-approval`.
   ...
   Then run `python3 -m cli.spawn_agent write '<json>'` with your designed
...
6. Run `python3 -m cli.spawn_agent resolve-handoff <agent-name>` to confirm
   the agent file is valid and ready.
